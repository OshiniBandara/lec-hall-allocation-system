...
function AdminSignup() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { name, employeeNumber } = formData;
      // Send POST request to server with form data
      const response = await axios.post('http://localhost:3000/api/users', { name, employeeNumber });
      if (response.status === 200) {
        setShowSuccessMessage(true);
        // Redirect to another page after successful signup
        history.push('/AdminLogin'); // Change the path to the appropriate page
      }
    } catch (error) {
   ...
   /* const isFormValid = Object.values(formData.validationErrors).every((error) => error === '');
    ...
    const cityRef = buddhi.collection('cities').doc("new-city-id");
